    ...
    protected void receiveInfo(GameInfo info) {
        ...
        if (cribGameState.getPlayerTurn() != this.playerNum) {
            ...
            for (int i = 0; i < cribGameState.getHandSize(this.playerNum); i++) {
                Log.d("SmartComputerPlayer cards: ", "" + cribGameState.getHandCard(this.playerNum, i).toString());
            }
            ...
            if (cribGameState.getHandSize(this.playerNum) == 0) { /* DEAL ACTION */
                ...
                CribEndTurnAction eta = new CribEndTurnAction(this);
                game.sendAction(eta);
                Log.d("SmartComputerPlayer", "has discarded: " + discardChoice[0].toString() + " " + discardChoice[1].toString());
                return;
            }
            ...
            /* PLAY CARD ACTION */
                ...
                int cardScore;
                Card card;
                int lastPlayedCardScore = cribGameState.getLastPlayedCard().getCardScore();
                int roundScore = cribGameState.getRoundScore(this.playerNum);
                Log.d("Last played card Score: ", "" + lastPlayedCardScore);

                //we want a good lead- first card they play in a round
                if (cribGameState.getInPlaySize() == 0) {

                    for (int i = 0; i < cribGameState.getHandSize(this.playerNum); i++) {

                        cardScore = cribGameState.getHandCard(this.playerNum, i).getCardScore();
                        card = cribGameState.getHandCard(this.playerNum, i);

                        if (cardScore == 4) {
                            //safest lead are 4's
                            CribPlayCardAction pca = new CribPlayCardAction(this, card);
                            game.sendAction(pca);
                            Log.d("SmartComputerPlayer", "played a 4 (hopefully): " + card.toString());

                            CribEndTurnAction eta = new CribEndTurnAction(this);
                            game.sendAction(eta);
                            return;
                        }
                        //save these to make 15's
                        else if (cardScore != 1 && cardScore != 2 && cardScore != 5 && cardScore != 10) {
                            //save aces, deuces, and 5's for later in play. -- too easy to make 15's & good for 21's and 31's
                            CribPlayCardAction pca = new CribPlayCardAction(this, card);
                            game.sendAction(pca);
                            Log.d("SmartComputerPlayer", "avoided playing an ace, two, five, or ten: " + card.toString());
                            ...
                            //just play a valid card
                            CribPlayCardAction pca = new CribPlayCardAction(this, cribGameState.getHandCard(this.playerNum, i));
                            game.sendAction(pca);
                            Log.d("SmartComputerPlayer", "played first available card: " + card.toString());
                ...
                //to make 15's with opponent's lead
                if (cribGameState.getInPlaySize() >= 1) {

                    //might be a first priority?????? - not rly
                    if (cribGameState.getPlayerSaidGoFirst() != -1) {
                        CribGoAction ga = new CribGoAction(this);
                        game.sendAction(ga);
                        Log.d("SmartComputerPlayer", "called \"Go\" for Opponent. ");
                    ...
                    for (int i = 0; i < cribGameState.getHandSize(this.playerNum); i++) {
                        ...
                        //to make 15's
                        if (lastPlayedCardScore + cardScore == 15 && cribGameState.isPlayable(card)) {
                            CribPlayCardAction pca = new CribPlayCardAction(this, card);
                            game.sendAction(pca);
                            Log.d("SmartComputerPlayer", "made a 15 with card: " + card.toString());
                            ...
                            //to make 31's
                        } else if (cardScore + roundScore >= 31 && cardScore + roundScore != 21 && cribGameState.isPlayable(card)) {

                            CribPlayCardAction pca = new CribPlayCardAction(this, card);
                            game.sendAction(pca);
                            Log.d("SmartComputerPlayer", "avoided 21 or made 31 with card: " + card.toString());
                    ...
                    //assuming none were done yet just go
                    if (cribGameState.getHandSize(this.playerNum) == 0
                            && cribGameState.getHandSize(1 - this.playerNum) != 0) {

                        CribGoAction ga = new CribGoAction(this);
                        game.sendAction(ga);
                        Log.d("SmartComputerPlayer", "said \"Go\". ");
        ...
        Log.d("SmartComputerPlayer", "is confused.");
        CribEndTurnAction eta = new CribEndTurnAction(this);
        game.sendAction(eta);
    }
